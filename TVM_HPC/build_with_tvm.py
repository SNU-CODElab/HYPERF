...
parser.add_argument("--library_path", type=str, required=True, help="Path to the shared library")
parser.add_argument("--tir_script_path", type=str, required=True, help="TIR script Path to be compiled")
parser.add_argument("--output_so_path", type=str, required=True, help="Output shared library path")
parser.add_argument("--params_path", type=str, required=False, help="Path to the input parameters")
    ...
    with open(file_path, 'r', encoding='utf-8') as file:  # utf-8 인코딩 적용
        for line in file:
            ...
            if match:
                ...
                shape = tuple(map(int, match.group(3).replace(" ", "").split(',')))  # 쉼표로 나누어 튜플 변환
                data_type = match.group(4)

                print(f"Function: {function_name}, Variable: {variable_name}, Shape: {shape}, Data Type: {data_type}")
...
def create_task_inputs(function_map, function_name):
    ...
    for variable_name, shape, data_type in function_map[function_name]:
        ...
        if data_type == "kDLFloat32":
            np_dtype = "float32"
            array = np.random.uniform(low=0.0, high=1.0, size=shape).astype(np_dtype)  # 랜덤 float 생성
        elif data_type == "kDLInt32":
            np_dtype = "int32"
            array = np.random.randint(low=0, high=10, size=shape, dtype=np_dtype)  # 랜덤 int 생성
            ...
            np_dtype = "int8"
            array = np.random.randint(low=0, high=10, size=shape, dtype=np_dtype)  # 랜덤 int8 생성
        elif data_type == "kDLUInt8":
            np_dtype = "uint8"
            array = np.random.randint(low=0, high=10, size=shape, dtype=np_dtype)
        elif data_type == "kDLUInt1":
            np_dtype = "bool"
            array = np.random.randint(low=0, high=2, size=shape, dtype=np_dtype)  # 랜덤 bool 생성
        else:
            print(f"Warning: Unsupported data type '{data_type}' for variable '{variable_name}'. Defaulting to float32.")
            np_dtype = "float32"
            array = np.random.uniform(low=0.0, high=1.0, size=shape).astype(np_dtype)
...
class ReplaceCallExternNamesPass:
    def __init__(self, old_names, new_names):
        assert len(old_names) == len(new_names), "old_names and new_names must have the same length"
    ...
    def transform_module(self, mod, ctx):
        ...
        def visitor(stmt):
            if isinstance(stmt, tir.Call) and stmt.op.name == "tir.call_extern":
                func_name = stmt.args[0]
                if isinstance(func_name, tir.StringImm) and func_name.value in self.name_map:
                    # Replace the function name with the mapped new name
                    new_func_name = tir.StringImm(self.name_map[func_name.value])
                    return tir.Call(stmt.dtype, stmt.op, [new_func_name] + stmt.args[1:])
            return stmt
        ...
        for func_name, func in mod.functions.items():
            if isinstance(func, tir.PrimFunc):
                ...
                new_func = tir.PrimFunc(func.params, new_body, func.ret_type, func.buffer_map, func.attrs)
...
with tempfile.TemporaryDirectory() as work_dir:
    ...
    target = Target("llvm --num-cores=64 -mcpu=sapphirerapids -opt-level=3")
    # target = Target("llvm --num-cores=64 -opt-level=3")
    start_time = time.time()
    print(f"Tuning start time: {start_time}")
    tuned_schedules = tune_ir_module(transformed_ir_module, target, work_dir, function_dict)
    end_time = time.time()
    ...
    for func_name, func in ir_module.functions.items():
        if func_name in tuned_schedules:
            ...
            if sch is not None:
                ...
                final_ir_module[func_name] = sch.mod[next(iter(sch.mod.get_global_vars()))]
            else:
                # 튜닝 실패한 경우 원래 함수 유지
...
# target = tvm.target.Target("llvm -opt-level=3")
    ...
    with tvm.transform.PassContext(opt_level=3):  
        mod = tvm.build(final_ir_module, target="llvm --num-cores=64 -opt-level=3 -mcpu=sapphirerapids", name="main")
