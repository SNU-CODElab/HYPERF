...
// 1. 초기 컴파일 & TIR 추출 단계
// ----------------------------
bool runInitialCompileAndExtractTIR(const std::string &inputCppPath,
                                    const std::string &compileopt) {
  // 기존 컴파일 명령어 + include path
  std::string command = "clang -fPIC -fopenmp -shared -g "
                        "-I/root/HYPERF/polybench/OpenMP/utilities " +
                        compileopt + " " + inputCppPath + " -o " +
                        inputCppPath + "_preprocess_tvm.so";
  ...
  if (ret == -1) {
    ...
    std::cerr << "Command failed with exit code: " << WEXITSTATUS(ret)
              << std::endl;
    return false;
  }
...
// ----------------------------
bool runTVMBuild(const std::string &libraryPath,
                 const std::string &tirScriptPath,
                 const std::string &outputSoPath,
                 const std::string &ParamsPath) {
  std::string command =
      "python3 /root/HYPERF/TVM_HPC/build_with_tvm.py --library_path " +
      libraryPath + " --tir_script_path " + tirScriptPath +
      " --output_so_path " + outputSoPath + " --params_path " + ParamsPath;
  std::cout << "Running: " << command << std::endl;
...
public:
  ...
  virtual void run(const MatchFinder::MatchResult &Result) override {
    ...
    if (const VarDecl *VD = Result.Nodes.getNodeAs<VarDecl>("varDecl")) {
      ...
      // 특정 varNames에만 적용
      if (std::find(varNames.begin(), varNames.end(), VarName) ==
          varNames.end()) {
        return;
      }
      ...
      if (!NewDecl.empty()) {
        ...
        llvm::StringRef OriginalText = Lexer::getSourceText(
            CharSourceRange::getCharRange(DeclRange), *Result.SourceManager,
            Result.Context->getLangOpts());
...
public:
  ...
  void run(const MatchFinder::MatchResult &Result) override {
    if (const BinaryOperator *BO =
            Result.Nodes.getNodeAs<BinaryOperator>("assignNewFloat")) {
      // RHS: new float[ ... ]
      const CXXNewExpr *NewE = Result.Nodes.getNodeAs<CXXNewExpr>("newExpr");
      if (!NewE)
        return;
...
public:
  ...
  void run(const MatchFinder::MatchResult &Result) override {
    if (const BinaryOperator *BO =
            Result.Nodes.getNodeAs<BinaryOperator>("assignMallocFloat")) {
      // RHS: malloc(...)
      ...
      StringRef ArgText = Lexer::getSourceText(
          CharSourceRange::getTokenRange(ArgE->getSourceRange()), SM, LangOpts);
      if (ArgText.empty())
        return;
...
public:
  ...
  void run(const MatchFinder::MatchResult &Result) override {
    if (const CXXDeleteExpr *DelE =
            Result.Nodes.getNodeAs<CXXDeleteExpr>("deleteExpr")) {
      // 삭제할 대상 포인터
      ...
      StringRef ArgText = Lexer::getSourceText(
          CharSourceRange::getTokenRange(Arg->getSourceRange()), SM, LangOpts);
      if (ArgText.empty())
        return;
...
public:
  OMPConsumer(Rewriter &R,
      ...
      : HandlerForTvm(R, ReplacementCodeMap), HandlerForAlignas(R, varNames),
        HandlerForNewFloatArray(R), HandlerForNewFloatAssignment(R),
        HandlerForMallocAssignment(R),
        HandlerForDelete(R) // << 추가: Delete 콜백
  {
    // 1) 배열 또는 기본형 varDecl에 alignas(64) 추가
    Finder.addMatcher(varDecl(hasType(type(anyOf(arrayType(), builtinType()))))
                          .bind("varDecl"),
                      &HandlerForAlignas);

    // 2) #pragma omp tvm 치환
    Finder.addMatcher(ompExecutableDirective().bind("ompAutotuneFor"),
                      &HandlerForTvm);

    // 3) (기존) float* A = new float[N]; (한 줄)
    Finder.addMatcher(
        varDecl(hasInitializer(
                    cxxNewExpr(hasType(pointsTo(asString("float"))),
    ...
    Finder.addMatcher(
        binaryOperator(
            hasOperatorName("="),
            hasRHS(cxxNewExpr(hasType(pointsTo(asString("float"))),
                              hasArraySize(expr().bind("arraySizeExpr")))
                       .bind("newExpr")))
            .bind("assignNewFloat"),
    ...
    Finder.addMatcher(
        binaryOperator(hasOperatorName("="),
                       hasRHS(callExpr(callee(functionDecl(hasName("malloc"))))
                                  .bind("mallocCall")))
            .bind("assignMallocFloat"),
        &HandlerForMallocAssignment);

    // 6) (추가) delete/delete[] -> std::free(...)
    Finder.addMatcher(cxxDeleteExpr().bind("deleteExpr"), &HandlerForDelete);
  }
...
public:
  OMPFrontendAction(const std::map<std::string, std::string> ReplacementCodeMap,
                    ...
                    const std::vector<std::string> &varNames,
                    std::string InputFilePath)
      : ReplacementCodeMap(ReplacementCodeMap), LoadCodeMap(LoadCodeMap),
        varNames(varNames), InputFilePath(InputFilePath) {}
...
#include <chrono>
    ...
    if (const RewriteBuffer *RewriteBuf =
      ...
      outFile.close();
      llvm::outs() << "Transformed source saved to" + InputFilePath +
                          "_replaced.cpp\n";
      ...
      // if (!tvmHome) {
      //     std::cerr << "TVM_HOME environment variable is not set." <<
      //     std::endl;
      // }
      ...
      // compileCmd += "-I" + std::string(tvmHome) +
      // "/3rdparty/dmlc-core/include "; compileCmd += "-I" +
      // std::string(tvmHome) + "/3rdparty/dlpack/include "; compileCmd += "-I"
      // + std::string(tvmHome) + "/3rdparty/rang/include "; compileCmd += "-I"
      // + std::string(tvmHome) + "/src/target/spirv "; compileCmd += "-L" +
      // std::string(tvmHome) + "/build -ltvm_runtime -ldl -o
      // transformed_output";
      ...
      // // 변환된 코드를 파이프로 전달
      // if (fwrite(transformedCode.data(), 1, transformedCode.size(),
      // clangPipe) != transformedCode.size()) {
      //     llvm::errs() << "Failed to write all code to clang++ stdin\n";
      // }

      // int status = pclose(clangPipe);
      // if (status == 0) {
      //     llvm::outs() << "Compilation successful. Run
      //     ./transformed_output\n";
      // } else {
  ...
  std::unique_ptr<ASTConsumer> CreateASTConsumer(CompilerInstance &CI,
    ...
    return std::make_unique<OMPConsumer>(TheRewriter, ReplacementCodeMap,
                                         varNames, InputFilePath);
  }
...
public:
  OMPFrontendActionFactory(const std::map<std::string, std::string> &code,
                           const std::map<std::string, std::string> &code_load,
                           const std::vector<std::string> &names,
                           std::string inputfilepath)
      : ReplacementCodeMap(code), LoadCodeMap(code_load), varNames(names),
        InputFilePath(inputfilepath) {}

  std::unique_ptr<clang::FrontendAction> create() override {
    return std::make_unique<OMPFrontendAction>(ReplacementCodeMap, LoadCodeMap,
                                               varNames, InputFilePath);
  }
};
...
std::string MakeLoadCode(const std::string &finalSoPath,
  ...
  loadCode += "tvm::runtime::PackedFunc f" + funcNameforCheck +
              " = mod.GetFunction(\"" + funcNameforCheck + "\");\n";
  return loadCode;
}
...
std::string MakeReplaceCode(const std::string &finalSoPath,
  ...
  while (getline(file, line)) {
    ...
    if (shapeVec.size() == 1 && shapeVec[0] == 1) {
      ...
      if (dtype == "kDLInt8")
        type_reg = "int8_t";
      else if (dtype == "kDLInt16")
        type_reg = "int16_t";
      else if (dtype == "kDLInt32")
        type_reg = "int32_t";
      else if (dtype == "kDLInt64")
        type_reg = "int64_t";
      else if (dtype == "kDLUInt8")
        type_reg = "uint8_t";
      else if (dtype == "kDLUInt16")
        type_reg = "uint16_t";
      else if (dtype == "kDLUInt32")
        type_reg = "uint32_t";
      else if (dtype == "kDLUInt64")
        type_reg = "uint64_t";
      else if (dtype == "kDLFloat16")
        type_reg = "float16_t";
      else if (dtype == "kDLFloat32")
        type_reg = "float";
      else if (dtype == "kDLFloat64")
        type_reg = "double";
      else if (dtype == "kDLBfloat")
        type_reg = "bfloat16_t";
      else if (dtype == "kDLOpaqueHandle")
        type_reg = "void*";
      // kDLUInt1
      else if (dtype == "kDLUInt1")
        type_reg = "uint8_t"; // 1비트는 uint8_t로 처리
      else if (dtype == "kDLFloat8")
        type_reg = "float8_t"; // float8은 별도의 타입으로 처리
      else if (dtype == "kDLFloat8E4M3")
        type_reg = "float8_e4m3_t"; // E4M3 포맷
      else if (dtype == "kDLFloat8E5M2")
        type_reg = "float8_e5m2_t"; // E5M2 포맷

      else {
        std::cerr << "Unsupported data type: " << dtype << std::endl;
        return "";
      }
      // replacementCode += varName + "_dltensor.data =
      // reinterpret_cast<void*>(const_cast<" + type_reg + "*>(&" + varName +
      // "));\n";

      replacementCode +=
          "tvm::runtime::NDArray " + varName +
          "_nd = tvm::runtime::NDArray::Empty({1}, DLDataType {" + baseType +
          "," + bitWidth + ",1}, dev);\n";
      replacementCode += "*reinterpret_cast<" + type_reg + "*>(" + varName +
                         "_nd.ToDLPack()->dl_tensor.data) = " + varName_org +
                         ";\n\n";
      ...
      replacementCode += varName + "_dltensor.data = reinterpret_cast<void*>(" +
                         varName_org + ");\n";
    } else {
      // 그 외(직접적인 배열? 등)
      replacementCode += "DLTensor " + varName + "_dltensor;\n";
    ...
    replacementCode +=
        varName + "_dltensor.ndim = " + std::to_string(shapeVec.size()) + ";\n";
    replacementCode += varName + "_dltensor.dtype = DLDataType {" + baseType +
                       "," + bitWidth + ",1};\n";
    ...
    replacementCode += "tvm::runtime::NDArray " + varName +
                       "_nd = tvm::runtime::NDArray::FromExternalDLTensor(" +
                       varName + "_dltensor);\n\n";
  }

  // replacementCode += "auto end_nd =
  // std::chrono::high_resolution_clock::now();\n"; replacementCode +=
  // "std::chrono::duration<double> elapsed_nd = end_nd - start_nd;\n";
  ...
  // replacementCode += "auto start =
  // std::chrono::high_resolution_clock::now();\n"; replacementCode += "int
  // measure_iters = 1000;\n"; replacementCode += "for (int m = 0; m <
  // measure_iters; m++) {\n";
  ...
  replacementCode += ");\n";
  // replacementCode += "}\n\n";
  // replacementCode += "auto end =
  // std::chrono::high_resolution_clock::now();\n"; replacementCode +=
  // "std::chrono::duration<double> elapsed = (end - start) / measure_iters;\n";

  // memcpy 처리
  // replacementCode += "auto start_post =
  // std::chrono::high_resolution_clock::now();\n";
  for (const auto &[varName, type_reg] : memcpy_vars) {
    replacementCode += memcpy_vars_org[varName] + "= *reinterpret_cast<" +
                       type_reg + "*>(" + varName +
                       "_nd.ToDLPack()->dl_tensor.data);\n";
  }
  // replacementCode += "auto end_post =
  // std::chrono::high_resolution_clock::now();\n"; replacementCode +=
  // "std::chrono::duration<double> elapsed_post = end_post - start_post;\n";

  // replacementCode += "std::cout << std::fixed << std::setprecision(12);\n";
  // replacementCode += "std::cout << \"전처리 시간  : \" << elapsed_nd.count()
  // + elapsed_post.count() << \" 초\\n\";\n"; replacementCode += "std::cout <<
  // \"평균 실행 시간: \" << elapsed.count() << \" 초\\n\";\n\n";
...
int main(int argc, const char **argv) {
  ...
  std::cout << "Initial compile and TIR extraction completed successfully."
            << std::endl;
  ...
  // auto OptionsParser = clang::tooling::CommonOptionsParser::create(argc,
  // argv, MyToolCategory);
  const char *args[] = {argv[0], argv[1]};
  int new_argc = 2;
  auto OptionsParser = clang::tooling::CommonOptionsParser::create(
      new_argc, args, MyToolCategory);
  if (!OptionsParser) {
  ...
  for (auto funcName : FuncNames) {
    ...
    std::string replacementCode =
        MakeReplaceCode(finalSoPath, paramsPath, varNames, funcName);
    std::string loadCode =
        MakeLoadCode(finalSoPath, paramsPath, varNames, funcName);
    // 중복제거해서 AllVarNames에 추가
    for (auto varName : varNames) {
      if (std::find(AllVarNames.begin(), AllVarNames.end(), varName) ==
          AllVarNames.end()) {
  ...
  Tool.appendArgumentsAdjuster(clang::tooling::getInsertArgumentAdjuster(
      ExtraArgs, clang::tooling::ArgumentInsertPosition::BEGIN));

  // 실제 액션 실행
  OMPFrontendActionFactory actionFactory(ReplacementCodeMap, LoadCodeMap,
                                         AllVarNames, inputCpp);
